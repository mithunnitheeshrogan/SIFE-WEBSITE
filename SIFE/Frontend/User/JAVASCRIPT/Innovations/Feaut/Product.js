document.addEventListener("DOMContentLoaded", function () {
    const FEAUT_API = 'http://localhost:5000/api/feaut'; // API endpoint for fetching data
    const container = document.getElementById('galert-container');
    
    async function fetchGAlertData() {
      try {
        const response = await fetch(FEAUT_API); // Fetch data from the API
        if (!response.ok) throw new Error('Failed to fetch FEAUT data');
        ...
        console.error(error);
        container.innerHTML = '<p class="error-message">Failed to load FEAUT data. Try again later.</p>';
      }
    }
